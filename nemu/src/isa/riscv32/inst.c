...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // INSTPAT("??????? ????? ????? 000 ????? 00100 11", li     , I, R(rd) = imm); //load immediate number, to be tested....
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti, I, if ((sword_t)src1 < (sword_t)imm) R(rd) = 1;
          else R(rd) = 0;); // set less than immediate
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu, I, if ((word_t)src1 < (word_t)imm) R(rd) = 1;
          else R(rd) = 0;);
  INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori, I, R(rd) = src1 ^ imm);
  INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori, I, R(rd) = src1 | imm);
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi, I, R(rd) = src1 & imm);

  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli, I, R(rd) = (word_t)src1 << (BITS(INSTPAT_INST(s), 24, 20)));
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli, I, R(rd) = (word_t)src1 >> (BITS(INSTPAT_INST(s), 24, 20)));
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai, I, R(rd) = (sword_t)src1 >> (BITS(INSTPAT_INST(s), 24, 20)));
  ...
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu, R, if ((word_t)src1 < (word_t)src2) R(rd) = 1;
          else R(rd) = 0;);
  ...
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu, R, R(rd) = BITS(1ll * (word_t)src1 * (sword_t)src2, 63, 32));
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu, R, R(rd) = BITS(1ull * (word_t)src1 * (word_t)src2, 63, 32));
