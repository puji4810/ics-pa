...
void ftrace_call(word_t pc, paddr_t target);
enum
{
  TYPE_I,
  TYPE_U,
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr, I, s->dnpc = (src1 + imm) & ~1; R(rd) = s->pc + 4;IFDEF(CONFIG_ITRACE,{
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = s->pc + imm; IFDEF(CONFIG_ITRACE, { 
    if (rd == 1) { // x1: return address for jumps(ra)
      ftrace_call(s->pc, s->dnpc);
    }
  }); R(rd) = s->pc + 4);
