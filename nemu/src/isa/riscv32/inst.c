...
static inline void decode_check_jal_callret(Decode *s, int rd){
    if (rd == 1) {
      s->jump_type = D_CALL;
    }
}

static inline void decode_check_jalr_callret(Decode *s, int rd, word_t imm){
  uint32_t i = s->isa.inst.val;
  int rs1 = BITS(i, 19, 15);
  if((rd == 0) && (rs1 == 1) && (imm == 0)){
    s->jump_type = D_RET;
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->snpc; s->dnpc = s->pc + imm; decode_check_jal_callret(s, rd));
  INSTPAT("??????? ????? ????? ??? ????? 11001 11", jalr   , I, R(rd) = s->snpc; s->dnpc = (src1 + imm) & ~((word_t) 1); decode_check_jalr_callret(s, rd, imm));
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = isa_raise_intr(MCAUSE_ENVIRONMENTCALL, s->pc));
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0

  // RV32M Standard Extension
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(rd) = src1 * src2);
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = ((dsword_t)(sword_t) src1 * (dsword_t)(sword_t) src2) >> (MUXDEF(CONFIG_ISA64, 64, 32)));
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = ((dword_t) src1 * (dword_t) src2) >> (MUXDEF(CONFIG_ISA64, 64, 32)));
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, {word_t t = get_csr(imm);set_csr(imm, src1);R(rd) = t;});
