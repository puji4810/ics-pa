...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // INSTPAT("??????? ????? ????? 100 ????? 01100 11", div, R, R(rd) = (int32_t)src1 / (int32_t)src2);
  // INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu, R, R(rd) = (uint32_t)src1 / (uint32_t)src2);
  // INSTPAT("??????? ????? ????? 110 ????? 01100 11", rem, R, R(rd) = (int32_t)src1 % (int32_t)src2);
  // INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu, R, R(rd) = (uint32_t)src1 % (uint32_t)src2);

  INSTPAT("0000000 ????? ????? 000 ????? 11001 11", ret, I, uint32_t t = s->pc + 4; s->dnpc = ((src1 + imm) & ~1); R(rd) = t);
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal, J, word_t t = s->pc + 4; s->dnpc = s->pc + imm; R(rd) = t);
