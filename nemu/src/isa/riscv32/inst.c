...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = Mr(src1 + imm, 2));
  INSTPAT("0000000 ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm, 1));
  INSTPAT("0000000 ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = SEXT(Mr(src1 + imm, 1), 8));
  INSTPAT("0000000 ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = SEXT(Mr(src1 + imm, 2), 16));
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", sltu   , I, R(rd) = (src1 < imm));
  ...
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = (sword_t)src1 >> BITS(imm, 5, 0));
  //INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(rd) = src1 << BITS(s->isa.inst.val, 25, 20));
  ...
  INSTPAT("??????? ????? ????? 101 ????? 01000 11", sd     , S, Mw(src1 + imm, 8, src2));
